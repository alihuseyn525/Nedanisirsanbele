...
function otaqYarat() {
  ...
  const otaq = {
    id, oyuncular:new Map(), komandalar:{A:[],B:[]},
    oyunVeziyyeti:'gozleme', mevcudKart:null, mevcudHerfler:[],
    mevcudKomanda:'A', xallar:{A:0,B:0}, turSayaci:0, maksimumTur:10,
    vaxt:60, vaxtInterval:null, cetinlik:'medium', kateqoriya:null,
    herfleSayi:6, secimler:[], guessHaqqi:3,
  };
  otaqlar.set(id, otaq);
  return otaq;
}
...
function otaqDurumunuGonder(otaq) {
  const base = {
    ...
    oyuncular:Array.from(otaq.oyuncular.values()).map(o=>({id:o.id,ad:o.ad,komanda:o.komanda,hazir:o.hazir,alien:o.alien})),
    komandalar:otaq.komandalar, oyunVeziyyeti:otaq.oyunVeziyyeti,
    xallar:otaq.xallar, turSayaci:otaq.turSayaci, maksimumTur:otaq.maksimumTur,
    ...
    guessHaqqi:otaq.guessHaqqi,
  };
  otaq.oyuncular.forEach(oyuncu => {
    const d = {...base};
    if (otaq.oyunVeziyyeti === 'oyun') {
      d.mevcudHerfler = otaq.mevcudHerfler;
...
function turBaslat(otaq) {
  ...
  if (komandaOyunculari.length) komandaOyunculari[(otaq.turSayaci-1)%komandaOyunculari.length].alien=true;
...
function oyunuBitir(otaq) {
  ...
  let qalib = otaq.xallar.A>otaq.xallar.B?'A':otaq.xallar.B>otaq.xallar.A?'B':'berabere';
  otaq.oyuncular.forEach(o=>{
    if (o.ws.readyState===1) o.ws.send(JSON.stringify({tip:'oyun_bitti',xallar:otaq.xallar,qalib}));
  });
}

app.get('/api/otaqlar',(req,res)=>{
  const list=[];
  otaqlar.forEach(o=>{ if(o.oyunVeziyyeti==='gozleme') list.push({id:o.id,oyuncuSayi:o.oyuncular.size,cetinlik:o.cetinlik,kateqoriya:o.kateqoriya}); });
  res.json(list);
});
  ...
  ws.on('message',(data)=>{
    ...
    switch(m.tip){
      case 'otaq_yarat':{
        ...
        const o={id:oyuncuId,ad:m.ad||'Oyunçu',komanda:'A',hazir:false,alien:false,ws};
      ...
      case 'otaqa_qosul':{
        ...
        const o={id:oyuncuId,ad:m.ad||'Oyunçu',komanda:k,hazir:false,alien:false,ws};
        otaq.oyuncular.set(oyuncuId,o); otaq.komandalar[k].push(oyuncuId);
        oyuncuOtaq=otaq;
        ws.send(JSON.stringify({tip:'qosuldu',otaqId:otaq.id,oyuncuId,komanda:k}));
        otaqDurumunuGonder(otaq); break;
      }
      case 'ayarlar_deyish':{
      ...
      case 'hazir':{
        ...
        if(o) o.hazir=true;
        if(Array.from(oyuncuOtaq.oyuncular.values()).every(x=>x.hazir)&&oyuncuOtaq.oyuncular.size>=2){
          oyuncuOtaq.oyunVeziyyeti='oyun'; turBaslat(oyuncuOtaq);
      ...
      case 'secim':{
        ...
        if(!guesser||guesser.alien||guesser.komanda!==oyuncuOtaq.mevcudKomanda) return;
        const dogru = m.secim===oyuncuOtaq.mevcudKart;
        if(dogru){
          oyuncuOtaq.xallar[oyuncuOtaq.mevcudKomanda]++;
          oyuncuOtaq.oyuncular.forEach(o=>{ if(o.ws.readyState===1) o.ws.send(JSON.stringify({tip:'secim_netice',secim:m.secim,dogru:true})); });
          // Vaxt bitməmişsə yeni söz gəlir, tur bitmir
      ...
      case 'yeniden_oyna':{
        if(!oyuncuOtaq) return;
        oyuncuOtaq.xallar={A:0,B:0}; oyuncuOtaq.turSayaci=0; oyuncuOtaq.oyunVeziyyeti='gozleme';
